...
static av_cold int init_filter(AVFilterContext *ctx, AVFrame *in)
{
    ...
    GLSLC(0, #extension GL_KHR_shader_subgroup_arithmetic : require  );
    GLSLF(0, shared vec4 sdata[%i];                                  ,vkctx->subgroup_props.maxSubgroupSize);
    ...
    GLSLC(0, vec4 sum(const ivec2 pos, const int idx)                     );
    GLSLC(0, {                                                            );

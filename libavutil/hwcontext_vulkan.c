                              ...
                              VkImageUsageFlags *supported_usage, int disable_multiplane)
{
    ...
    for (int i = 0; i < nb_vk_formats_list; i++) {
        if (vk_formats_list[i].pixfmt == p) {
            ...
            } else if (full && basics && !disable_multiplane) {
                if (fmts)
...
static int vulkan_frames_init(AVHWFramesContext *hwfc)
{
    ...
    err = vkfmt_from_pixfmt2(hwfc->device_ctx, hwfc->sw_format, 0, /* drivers must fix feats. */
    ...
    if (fp->fmt->vk_planes > 1 && !disable_multiplane)
        // || supported_usage & VK_IMAGE_USAGE_VIDEO_ENCODE_SRC_BIT_KHR)
                              ...
                              int h, enum AVPixelFormat pix_fmt, int to_buf)
{
    ...
    for (int i = 0; i < pixfmt_planes; i++) {
        ...
        VkBufferImageCopy buf_reg = {
            ...
            .imageSubresource.aspectMask = plane_aspect[(img_planes != nb_images) +
                                                        i*(img_planes != nb_images)],
            .imageOffset = { 0, 0, 0, },
        };

...
static int vulkan_frames_init(AVHWFramesContext *hwfc)
{
    ...
    if (p->use_linear_images &&
        (hwctx->tiling == VK_IMAGE_TILING_OPTIMAL &&
                                ...
                                const AVFrame *swf, int from)
{
    ...
    AVFrame tmp;
    uint8_t **buf_data[AV_NUM_DATA_POINTERS];
    ...
    for (int i = 0; i < planes; i++) {
        ...
        buf_data[i] = &tmp.data[i];
    }

    if (!from) {
        /* Map, copy image TO buffer (which then goes to the VkImage), unmap */
        if ((err = ff_vk_map_buffers(&p->vkctx, vkbufs, buf_data, planes, 0)))
            goto end;

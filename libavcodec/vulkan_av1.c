                            ...
                            int dpb_slot_index)
{
    ...
    int err = ff_vk_decode_prepare_frame(ctx, pic->f, vkpic,
                                         is_current, has_grain, dpb_slot_index);
    if (err < 0)
        return err;
    ...
    StdVideoAV1SequenceHeader av1_sequence_header = {
        ...
        .color_config = (StdVideoAV1ColorConfig) {
            .flags = (StdVideoAV1ColorConfigFlags) {
                ...
                .color_description_present_flag = seq->color_config.color_description_present_flag,
                              ...
                              av_unused uint32_t       size)
{
    ...
    ap->av1_frame_header = (StdVideoAV1FrameHeader) {
        ...
        .last_frame_idx = frame_header->last_frame_idx,
        ...
        .tiling = (StdVideoAV1Tiling) {
            ...
            .tile_cols_log2 = frame_header->tile_cols_log2,
            .tile_rows_log2 = frame_header->tile_rows_log2,
        ...
        .loop_filter = (StdVideoAV1LoopFilter) {
            ...
            .update_mode_delta = {
                frame_header->update_mode_delta[0], frame_header->update_mode_delta[1],
            },
    ...
    for (int i = 0; i < 32; i++)
        ap->av1_frame_header.buffer_removal_time[i] = frame_header->buffer_removal_time[i];

    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            ap->av1_frame_header.segmentation.feature_enabled[i][j] = frame_header->feature_enabled[i][j];
            ap->av1_frame_header.segmentation.feature_value[i][j] = frame_header->feature_value[i][j];
        }

        ap->av1_frame_header.loop_filter.update_ref_delta[i] = frame_header->update_ref_delta[i];
        ap->av1_frame_header.loop_filter.loop_filter_ref_deltas[i] = frame_header->loop_filter_ref_deltas[i];

        ap->av1_frame_header.cdef.cdef_y_pri_strength[i] = frame_header->cdef_y_pri_strength[i];
        ap->av1_frame_header.cdef.cdef_y_sec_strength[i] = frame_header->cdef_y_sec_strength[i];
        ap->av1_frame_header.cdef.cdef_uv_pri_strength[i] = frame_header->cdef_uv_pri_strength[i];
    ...
    for (int i = 0; i < 7; i++) {
        ap->av1_frame_header.found_ref[i] = frame_header->found_ref[i];

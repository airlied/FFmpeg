                       ...
                       AVFrame *rpic[], FFVulkanDecodePicture *rvkp[])
{
    ...
    ff_vk_exec_update_frame(&ctx->s, exec, pic->buf[0],
    ...
    if (!layered_dpb) {
        ...
        for (int i = 0; i < vp->decode_info.referenceSlotCount; i++) {
            ...
            AVBufferRef *ref = rvp->dpb_ref ? rvp->dpb_ref : ref_frame->buf[0];
                                    ...
                                    enum AVPixelFormat *pix_fmt, int *dpb_dedicate)
{
    ...
    VkVideoDecodeAV1ProfileInfoMESA *av1_profile = ctx ? &ctx->av1_profile : &local_av1_profile;

                           ...
                           uint32_t *nb_slices, const uint32_t **offsets)
{
    ...
    size_t new_size = vp->slices_size + startcode_len +
...
repeat:
    ...
    if (!pix_fmt) {
        ...
        for (int i = 0; i < nb_out_fmts; i++) {
            ...
            if (tmp != AV_PIX_FMT_NONE && tmp == frames->sw_format)
                found = 1;
        }
        av_free(ret_info);
        if (!found) {
            av_log(avctx, AV_LOG_ERROR, "Unable to find the pixel format %s set "
                   "in the output frames context!\n",
